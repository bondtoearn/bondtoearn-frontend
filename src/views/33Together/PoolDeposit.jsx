...
export const PoolDeposit = props => {
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      t`Deposit sBTE to win! Once deposited, you will receive a corresponding amount of 33T and be entered to win until your sBTE is withdrawn.`,
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            {address && hasAllowance("sohm") ? (
              <Button
                ...
                style={{ margin: "5px" }}
              >
                {txnButtonText(pendingTransactions, "pool_deposit", t`Deposit sBTE`)}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
          ...
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography variant="body2" style={{ color: "#33BB33" }}>
                <Trans>
                  Depositing {quantity} sBTE will increase odds of winning to 1 in {newOdds}
                </Trans>
                &nbsp;
              </Typography>
            </Box>
          )}
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had sBTE in during testing */}
            ...
            <div className="data-row">
              ...
              <Typography variant="body1" align="right">
                {isAppLoading ? (
                  ...
                  <>{new Intl.NumberFormat("en-US").format(sohmBalance)} sBTE</>
                )}
              </Typography>
            </div>
          </div>
